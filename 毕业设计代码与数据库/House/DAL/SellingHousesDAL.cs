...
namespace DAL
{
    public class SellingHousesDAL
    {
        ...
        {
            return db.Sell.Include("Userd").Where(p => p.UState == 0 && p.TransactionStatus == 1).ToList();
        }
        /// <summary>
        /// 查询详情
        /// </summary>
        ...
        {
            //降序排列
            return db.Sell.Where(p => p.UState == 0&&p.TransactionStatus==1).OrderByDescending(p => p.SellPrice).ToList();
        }
        /// <summary>
        /// 接个排序从低到高
        /// </summary>
        ...
        {
            //升序排列
            return db.Sell.Where(p => p.UState == 0 && p.TransactionStatus == 1).OrderBy(p => p.SellPrice).ToList();
        }
        /// <summary>
        /// 最新排序
        /// </summary>
        ...
        {
            //降序排列
            return db.Sell.Where(p => p.UState == 0 && p.TransactionStatus == 1).OrderByDescending(p => p.NewTime).ToList();
        }
        /// <summary>
        /// 最旧排序
        /// </summary>
        ...
        {
            //升序排列
            return db.Sell.Where(p => p.UState == 0 && p.TransactionStatus == 1).OrderBy(p => p.NewTime).ToList();
        }
        /// <summary>
        /// 查询关注
        /// </summary>
